...
function displayData(data) {
  ...
  if (data.length === 0) {
      resultsContainer.innerHTML = '<p>Nincs ilyen találat.</p>';
      return;
  }
  ...
  const avgMatematika = sumMatematika / data.length;
  const avgMagyar = sumMagyar / data.length;
  const avgCombined = avgMatematika + avgMagyar;

  let tableHtml = `<table class="table-style">
      <tr>
        <th onclick="sortData('OM_Azonosito')">OM Azonosító</th>
        <th onclick="sortData('Neve')">Neve</th>
        <th onclick="sortData('ErtesitesiCime')">ErtesitesiCíme</th>
        <th onclick="sortData('Email')">Email</th>
        <th onclick="sortData('SzuletesiDatum', true)">Születési Dátum</th>
        <th onclick="sortData('Matematika')">Matematika</th>
        <th onclick="sortData('Magyar')">Magyar</th>
      </tr>`;

  data.forEach(item => {
      tableHtml += `<tr>
          <td>${item.OM_Azonosito}</td>
          <td>${item.Neve}</td>
          <td>${item.ErtesitesiCime}</td>
          <td>${item.Email}</td>
          <td>${item.SzuletesiDatum.split('T')[0]}</td>
  ...
  window.sortData = function(sortKey, isDate = false) {
    data.sort((a, b) => {
        if(isDate) {
            return new Date(a[sortKey]) - new Date(b[sortKey]);
        } else if (!isNaN(a[sortKey]) && !isNaN(b[sortKey])) {
